...
function log_only(){
    ...
    if (!isset($_SESSION['auth'])) {
        $_SESSION['flash2']['danger'] = "Vous n'avez pour le moment pas le droit d'acceder a cette partie <br> Veuillez vous connecter pour allé plus loin";
...
function log_only_first(){
    if (session_status() == PHP_SESSION_NONE){
        session_start();
    }

    if (($_SESSION['auth']->firstlog_bool) == 1) {
        header('Location: ../index.php');
        exit();
    }
    /*
